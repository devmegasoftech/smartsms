...
<div class="fieldcontain ${hasErrors(bean: articulosInstance, field: 'idotroarticulo', 'error')} ">
	<label for="idotroarticulo">
		<g:message code="articulos.idotroarticulo.label" default="Idotroarticulo" />
		
	</label>
	<g:select id="idotroarticulo" name="idotroarticulo.id" from="${smartsms.Articulos.list()}" optionKey="id" value="${articulosInstance?.idotroarticulo?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>
